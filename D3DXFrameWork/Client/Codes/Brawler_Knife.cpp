#include "stdafx.h"
#include "..\Headers\Brawler_Knife.h"
...
CBrawler_Knife::CBrawler_Knife(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CBrawler_Knife::CBrawler_Knife(const CBrawler_Knife & rhs)
	: CGameObject(rhs)
{
}

HRESULT CBrawler_Knife::Ready_GameObject_Prototype()
{
	return NOERROR;
}

HRESULT CBrawler_Knife::Ready_GameObject()
{
...
_int CBrawler_Knife::Update_GameObject(const _float & fTimeDelta)
{
...
_int CBrawler_Knife::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
void CBrawler_Knife::Render_GameObject()
{
...
HRESULT CBrawler_Knife::Ready_Component()
{
	...
	// For.Com_Mesh
	m_pMeshCom = (CMesh_Static*)pComponent_Manager->Clone_Component(SCENE_STAGE, L"Component_Mesh_Brawler_Knife");
...
HRESULT CBrawler_Knife::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect)
		return E_FAIL;
...
HRESULT CBrawler_Knife::SetUp_ParentPointer(CBrawler* pBrawler)
{
	m_pBrawler = pBrawler;
	if (nullptr == m_pBrawler)
		return E_FAIL;
...
CBrawler_Knife * CBrawler_Knife::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CBrawler_Knife*		pInstance = new CBrawler_Knife(pGraphic_Device);
...
CGameObject * CBrawler_Knife::Clone_GameObject()
{
	CBrawler_Knife*		pInstance = new CBrawler_Knife(*this);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CBrawler_Knife Clone Failed");
		Safe_Release(pInstance);
	}

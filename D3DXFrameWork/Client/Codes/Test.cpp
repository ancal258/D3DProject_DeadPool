...
HRESULT CTest::Ready_GameObject_Prototype()
{
	return NOERROR;
}
...
HRESULT CTest::Ready_Component()
{
	...
	// For.Com_Mesh
	m_pMeshCom = (CMesh_Static*)pComponent_Manager->Clone_Component(SCENE_STAGE, L"Component_Mesh_Stone");
	...
	m_pColliderCom->SetUp_Collider(m_pMeshCom->Get_MinPoint(), m_pMeshCom->Get_MaxPoint(), m_pTransformCom->Get_WorldMatrix_NotRot());//, &vTransMore);
...
HRESULT CTest::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect)
		return E_FAIL;

...
HRESULT CSound_Manager::Ready_Sound_Manager(void)
{
	//사운드 디바이스 생성
...
//사운드파일을 읽기 위한함수(경로)
HRESULT CSound_Manager::Add_SoundWaveFile(TCHAR* pFileName)
{
	HMMIO	hFile;
...
void CSound_Manager::SoundPlay(int iIndex, DWORD dwFlag)
{
...
void CSound_Manager::SoundStop(int iIndex)
{
	if (iIndex < 0 || iIndex >(signed)m_vecSoundBuff.size())
		return;
...
bool CSound_Manager::SoundPlaying(int iIndex)
{
	DWORD	dwStatus = 0;

...
CEffect_GunCapT::CEffect_GunCapT(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CGameObject(pGraphic_Device)
{
}

CEffect_GunCapT::CEffect_GunCapT(const CEffect_GunCapT & rhs)
	: CGameObject(rhs)
{
}

HRESULT CEffect_GunCapT::Ready_GameObject_Prototype()
{
	return NOERROR;
}

HRESULT CEffect_GunCapT::Ready_GameObject()
{

	if (FAILED(CEffect_GunCapT::Ready_Component()))
		return E_FAIL;
...
_int CEffect_GunCapT::Update_GameObject(const _float & fTimeDelta)
{
	m_fU += m_fSpeed_U * g_fTimeDelta;
	m_fV += m_fSpeed_V * g_fTimeDelta;

	m_pTransformCom->RotationZ(360, fTimeDelta);
...
_int CEffect_GunCapT::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
void CEffect_GunCapT::Render_GameObject()
{
...
HRESULT CEffect_GunCapT::Ready_Component()
{
	...
	// For.Com_Mesh
	m_pMeshCom = (CMesh_Static*)pComponent_Manager->Clone_Component(0, L"Component_Mesh_MinigunCap");
...
HRESULT CEffect_GunCapT::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect)
		return E_FAIL;
...
CEffect_GunCapT * CEffect_GunCapT::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CEffect_GunCapT*		pInstance = new CEffect_GunCapT(pGraphic_Device);
...
CGameObject * CEffect_GunCapT::Clone_GameObject()
{
	CEffect_GunCapT*		pInstance = new CEffect_GunCapT(*this);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CEffect_GunCapT Clone Failed");
		Safe_Release(pInstance);
	}
...
HRESULT CEffect_GunCapT::Update_HandMatrix()
{

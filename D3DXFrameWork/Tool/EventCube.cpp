...
CEventCube::CEventCube(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CGameObject(pGraphic_Device)
{
}

CEventCube::CEventCube(const CEventCube & rhs)
	: CGameObject(rhs)
{
}

_vec3 CEventCube::Get_ObjectInfo()
{
	return *m_pTransformCom->Get_StateInfo(CTransform::STATE_POSITION);
}

void CEventCube::Set_StateInfo(_vec3 * pRight, _vec3 * pUp, _vec3 * vLook, _vec3 * vPos)
{
...
void CEventCube::Set_Position(_vec3 * pPosition) const
{
	m_pTransformCom->Set_StateInfo(CTransform::STATE_POSITION, pPosition);
}

void CEventCube::Set_Scale(_vec3 * pScale) const
{
	m_pTransformCom->Scaling(*pScale);
}

void CEventCube::Set_PlusPosition(_uint iType, _float & fValue) const
{
	m_pTransformCom->Set_PlusPosition(iType, fValue);
}

void CEventCube::RotationY(const _float & fRadian) const
{
	m_pTransformCom->RotationY(fRadian, 1);
}

void CEventCube::Scaling(_float fX, _float fY, _float fZ) const
{
	m_pTransformCom->Scaling(fX, fY, fZ);
}


HRESULT CEventCube::Ready_GameObject_Prototype()
{
	return NOERROR;
}
...
_int CEventCube::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
HRESULT CEventCube::Ready_Component()
{
...
HRESULT CEventCube::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect)
		return E_FAIL;
...
CEventCube * CEventCube::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CEventCube*		pInstance = new CEventCube(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		_MSG_BOX("CEventCube Created Failed");
...
CGameObject * CEventCube::Clone_GameObject()
{
	CEventCube*		pInstance = new CEventCube(*this);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CEventCube Clone Failed");
		Safe_Release(pInstance);
	}
